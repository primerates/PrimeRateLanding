...
### Admin Features
- **Comments & Posts Management**: System for client testimonials and internal company posts with 4-tab navigation (Company Post, All Posts, Posts, Notes), real-time preview, statistics, localStorage persistence, sortable tables, and sticky notes functionality. The "Last Comment" and "Posts" cards (with circular indicators and 5 checkboxes: Client Comment, All Comments, Company Post, All Posts, Notes) are only visible when the "Posts" tab is active. The Client Comment form card and All Comments table both appear with smooth roll-down animations below the Posts cards when their respective checkboxes are clicked in the Posts tab, and each toggles open/close on every click.
