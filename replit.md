...
### Admin Features
- **Comments & Posts Management**: System for client testimonials and internal company posts with 2-tab navigation (Company Post, Notes), real-time preview, statistics, localStorage persistence, sortable tables, and sticky notes functionality. The "Last Comment" and "Posts" cards (with circular indicators and 5 checkboxes: Client Comment, All Comments, Company Post, All Posts, Notes) are always visible. The Client Comment form card, All Comments table, and All Posts table all appear with smooth roll-down animations below the Posts card when their respective checkboxes are clicked, and each toggles open/close on every click.
