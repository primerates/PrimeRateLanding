...
### Admin Features
- **Comments & Posts Management**: System for client testimonials and internal company posts with 5-tab navigation (All Comments, Company Post, All Posts, Posts, Notes), real-time preview, statistics, localStorage persistence, sortable tables, and sticky notes functionality. The "Last Comment" and "Posts" cards (with circular indicators and 5 checkboxes: Client Comment, All Comments, Company Post, All Posts, Notes) are only visible when the "Posts" tab is active. The Client Comment form card appears with a smooth roll-down animation below the Posts cards when the "Client Comment" checkbox is clicked in the Posts tab, and toggles open/close on each click.
